...
class OrderTrackView(APIView):
    def post(self, request):
        ...
        #### 2
        # 모든 Payment 인스턴스에서 tid 값을 가져옵니다.
        payment_records = Payment.objects.filter(user=user)  # 특정 사용자에 대한 결제 기록 필터링
        responses = []  # 응답 리스트 초기화
        ...
        for payment in payment_records:
            ...
            try:
                response = requests.post(order_url, headers=pay_header, data=pay_data)

                # 응답이 성공적인지 확인
                responses.append(response.json())  # 응답을 리스트에 추가

            except Exception as e:
                # 예외 처리 (예: 요청 실패 등)
                print(f"Error while processing tid {payment.tid}: {e}")
                responses.append({"detail": f"Error processing tid {payment.tid}"})

        return Response(responses, status=status.HTTP_200_OK)  # 모든 응답을 리스트로 반환